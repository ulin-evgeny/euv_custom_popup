 ...
 * Плагин для вызова кроссбраузерного, адаптивного, стилизованного попапа, размещенного по середине экрана, который можно скроллить.
 */
...
(function ($) {
    ...
    let methods = {
         ...
         * Для восстановления содержимого используются соответствующие названию функции из JQuery (либо вообще не используются - если значение 'none').
         * Важный момент! Из-за clone обработчики, повешенные на какой-либо элемент, могут восстановиться некорректно. Пример: https://codepen.io/codepen_user_2020/pen/VwwXWjm . Поэтому, если данная настройка активирована, вешать обработчики рекомендуется через функцию before_open_function (в момент открытия попапа, а не до открытия или еще раньше).
         *
